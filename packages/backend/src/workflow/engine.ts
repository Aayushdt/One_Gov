...
// Task 20: Demo pacing is now configurable via DEMO_PACING_MS env var (default: 0 = off).
// Set DEMO_PACING_MS=600 in .env to restore presentation pacing without touching core logic.
const DEMO_PACING_MS = Number(process.env.DEMO_PACING_MS ?? 0);
...
export class WorkflowEngine {
  // Task 21: Refactored from recursive await this.advance(runId) tail calls
  // to an iterative while-loop to eliminate stack overflow risk on deep workflows.
  async advance(runId: string): Promise<void> {
    // Loop until we reach a terminal or async-exit condition
    while (true) {
      ...
      try {
        // 1. Initial State: Awaiting Consent → transition to first connector step
        if (run.state === WorkflowState.AWAITING_CONSENT) {
          ...
          continue; // iterate — do not recurse
        }
        ...
        if (currentStepIndex === -1) {
          // All steps have snapshots — proceed to eligibility calculation
        ...
        continue; // iterate — process next state in next loop iteration
        ...
        if (err instanceof ConnectorError && RETRY_POLICY.retryableStatusCodes.includes(err.statusCode)) {
          ...
          if (newRetryCount < RETRY_POLICY.maxAttempts) {
            ...
            return; // async retry — exit loop, BullMQ will re-enqueue
          }
        }
